...
from .autocomplete import get_completion, get_context_and_prefix


# Color pair for ghost text (autocomplete suggestions)
COLOR_GHOST = 12
...
def init_colors():
    ...
    curses.init_pair(COLOR_GHOST, curses.COLOR_WHITE, curses.COLOR_BLACK)  # ghost text (dim)
    return True
...
class IDE:
    ...
    def get_ghost_text(self) -> str:
        """Get the autocomplete suggestion for the current cursor position."""
        line = self.lines[self.cursor_y]
        context, prefix = get_context_and_prefix(line, self.cursor_x)
        if prefix or context:
            suggestion = get_completion(context, prefix)
            return suggestion
        return ""
    ...
    def accept_ghost_text(self):
        ...
        if ghost:
            line = self.lines[self.cursor_y]
            self.lines[self.cursor_y] = line[:self.cursor_x] + ghost + line[self.cursor_x:]
            self.cursor_x += len(ghost)
            self.modified = True
    ...
    def draw(self):
        ...
        # Draw ghost text (autocomplete suggestion) after cursor
        ghost = self.get_ghost_text()
        if ghost and 0 <= (self.cursor_y - self.scroll_y) < h - 2:
            cursor_screen_y = self.cursor_y - self.scroll_y
            cursor_screen_x = self.cursor_x - self.scroll_x + line_num_width
            ghost_display = ghost[:max(0, w - cursor_screen_x - 1)]
            if ghost_display and cursor_screen_x < w:
                try:
                    self.stdscr.addstr(cursor_screen_y, cursor_screen_x, ghost_display,
                                       curses.color_pair(COLOR_GHOST) | curses.A_DIM)
                except curses.error:
                    pass
    ...
    def run(self):
        ...
        while True:
            ...
            if ch == curses.KEY_UP:
                ...
                self.insert_newline()
            elif ch == 9:  # Tab - accept autocomplete or insert spaces
                ghost = self.get_ghost_text()
                if ghost:
